...
- A reachable recent `llama-server` build that does not require HTTP authentication.
...
Enter the base URL of `llama-server`, for example `http://192.168.1.10:8080`. Do **not** append `/v1`; the integration adds the endpoint paths itself.
...
- The integration does not send API keys or other HTTP authorization credentials; use a llama-server endpoint that is reachable without authentication.
...
| --- | --- |
| Failed to connect during setup | `llama-server` is not reachable from Home Assistant, requires authentication, is bound only to loopback, or a firewall is blocking it. |
