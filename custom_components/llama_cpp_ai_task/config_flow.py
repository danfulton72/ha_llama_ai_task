...
def _is_autogenerated_task_title(title: str, old_model: Any) -> bool:
    """Return whether a task title is still one of our generated defaults."""
    known_auto_titles = {DEFAULT_AI_TASK_NAME, model_name_to_title("llama.cpp")}
    if old_model:
        known_auto_titles.add(model_name_to_title(str(old_model)))
    return title in known_auto_titles
...
class LlamaCppSubentryFlowHandler(ConfigSubentryFlow):
    ...
    async def async_step_set_options(
        ...
        if user_input is not None:
            ...
            # user-customized title intact when the model changes. The generic
            # router fallback "Llama.cpp" is autogenerated too, so it remains
            # self-correcting when a real model is selected later.
            title = (
                new_auto_title
                if _is_autogenerated_task_title(subentry.title, old_model)
                else subentry.title
            )
